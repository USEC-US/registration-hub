...
Issuance checks tournament publication, registration dates, fees, and capacity, but does not reserve a roster or capacity slot. Responses are not cacheable and issuance is throttled. Legacy clients may retain the private UUID claim token for quote/template compatibility. The staged browser no longer issues a pre-submission quote. It detects tokens left by the earlier form in division-scoped session storage. A later tournament edit does not rewrite instructions already issued. The token attaches an intent atomically to one registration. Fee changes block submission and require organizer assistance; rejected content does not consume the intent. Codes are not expired or recycled.
...
Existing intents that predate transfer snapshots retain blank content/template fields: the migration does not invent what participants were previously told. The compatibility API continues returning `legacy_payment_session` for those unattached quotes. Existing submitted records with blank content can still upload proof, with organizer guidance if instructions are needed. Historical `PaymentAttempt.reference` values remain private manual transaction notes. Legacy registrations without any intent can receive one through the owner endpoints.

The actual staged registration route checks `sessionStorage['usec-payment-intent:<division>']` before displaying a fresh form. Any saved earlier quote triggers organizer guidance for participants who already transferred and an explicit **I haven’t paid — start a new registration** choice. It does not fetch today's destination into the old quote. The choice belongs only to this mounted division: reloading, leaving, or changing divisions requires a new choice. Ordinary new submissions still contain neither initial proof nor an intent token.

The old browser token remains on validation failures and uncertain responses. It is removed only after the explicitly chosen fresh attempt succeeds (including confirmed same-instance lost-response recovery), and only if its value still matches. The old database quote remains untouched. Saved entries can be recovered without declaring unpaid: read-only resume is allowed, but a missing submission cannot be replayed until the participant explicitly chooses an unpaid start. Reload loses the in-memory association between an earlier quote and a fresh attempt, so a successful read-only resume conservatively retains the old token; it never clears a quote merely because another saved registration exists. An explicitly chosen replay that directly succeeds can clear its matching token. If that replay itself loses its response, later read-only recovery likewise conservatively retains the old quote. Participants who already transferred should recover their saved submitted entry if available or contact organizers with their proof.
