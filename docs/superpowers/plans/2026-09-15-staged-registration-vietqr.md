...
**Spec:** [Approved design](../specs/2026-09-15-staged-registration-vietqr-design.md). The user approved it in conversation on 2026-09-15; that approval supersedes its remaining “Proposed design” label.

## Global constraints

- Implement only after the user resumes implementation. Do not install dependencies, migrate databases, seed data, or run implementation tests as part of writing this plan. A branch is required for safety.
...
- [ ] **Write failing encoder and settings tests.** Start with an independently known checksum and a complete payload vector; keep these tests independent of Django database setup:
...
- [ ] **Write deterministic deadline tests** using `RegistrationOwnershipApiTests.setUp`, which supplies published division, owner, and historical registration fixtures. Example method for a new TestCase that reuses that setup:
...
- [ ] **Run the failing reservation tests**, then implement the shared predicate. Use `Exists` to avoid multiplying registrations when several attempts exist:
...
- [ ] **Write a failing end-to-end API contract test.** Reuse the fixture setup and `payload()` from `GuestSubmissionTests`, enable a test-only `PaymentSettings`, and submit without initial proof:
...
- [ ] **Test observable recovery behavior.** Cover 201 and replay 200, uncertain submit then successful resume, unknown-credential retry using the same key, changed payload requiring resolution, auth expiry, quota failure, two entries in one division, clearing a draft without clearing submitted access, and forgetting only one entry. API tests assert header/body routing and response/error handling with complete response fixtures; storage tests use real parsing/storage logic.
- [ ] **Run focused Vitest and Svelte checks**, then commit the reviewed browser storage and API-client files. No component changes belong in this task.
...
All tasks are planned, not started. The next action when the user resumes implementation is execution setup followed by Task 1's failing tests. There is no implementation or deployment approval request attached to writing this plan.
