...
export class AuthState {
	...
	async initialize(): Promise<CurrentUser | null> {
		...
		this.status = 'loading';
		const promise = getCurrentUser(accessToken)
			.then((user) => {
				if (!this.isActiveInitialization(accessToken, generation, operationGeneration)) return null;
			...
			.catch((error: unknown) => {
				if (this.isActiveInitialization(accessToken, generation, operationGeneration)) {
		...
		const initialization = { accessToken, generation, operationGeneration, promise };
